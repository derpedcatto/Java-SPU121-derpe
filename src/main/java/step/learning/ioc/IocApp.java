...
public class IocApp {
    ...
    public void demo() {
        ...
        System.out.println("Hash of " + str + " = " + hashService2.hash(str));
...
- перед точкою інжекції слід зазначати анотацію @Inject (import com.google.inject.Inject;)
- інжекція у приватні поля працює
- якщо клас залежності 'видимий' у проєкті, то інжектор його знаходить автоматично
- за замовченням залежності впроваджуються як Transient (тобто новий об'єкт
у кожній точці інжекції). Альтернативний варіант - Singleton (коли всі точки
інжекції отримають один об'єкт зазначається анотацією до класу залежності (сервісу))
...
- DIP рекомендує створювати інтерфейси і зазначати залежності від них, що
спрощує зміну служб без втручання у код точок інжекції. Але при цьому
вимагається "зв'язування" (binding) інтерфейсів та класів, що їх реалізують.
Це здійснюється у конфігураційних модулях (класах)
- За потреби кількох реалізацій одного типу (інтерфейсу) вживаються іменовані
залежності. Точки інжекції анотуються @Named("Hash-160"), а інструкції
зв'язування доповнюються посиланням на іменування
